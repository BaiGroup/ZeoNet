...
class ZeoImgsDataset(Dataset):
    ''' Zeolites dataset for mvcnn'''

    def __init__(self, root_dir, grid_size=100, grid_resolution=0.45, base_grid_resolution=0.15, num_views=1,is_train=True, is_val=False, **kwargs):
        ...
        for i in range(num_views):
            seed(i)
            axis = randint(0, 2) # axis
            i = randint(0, grid_size-1-2*a) # slice in the axis
            indices = [i, i+a, i+2*a] # indices in the axis
            views.append([axis] + indices) # [axis, i, i+a, i+2*a], length is number of views
    ...
    def __getitem__(self, idx):
        ...
        grid_res = grid_res.squeeze() # [100, 100, 100]
        mulit_views = torch.zeros((len(self.views), 3, self.grid_size, self.grid_size))
        for i in range(len(self.views)):
            mulit_views[i,:,:,:] = self.volume2views(grid=grid_res, grid_size=self.grid_size, view=self.views[i], is_train=self.is_train)
        
        sample = {
                  'image': mulit_views, # [# of views,3,100,100]
    ...
    def volume2views(self, grid, grid_size, view, a=3, is_train=False):
        single_view = torch.zeros((3, grid_size, grid_size))
        if is_train:
            axis = randint(0, 2)
            i = randint(0, grid_size-1-2*a)
            indices = [i, i + a, i + 2*a]
        else:
            axis = view[0]
            indices = [view[1], view[2], view[3]] 
   
        for n, idx in enumerate(indices):
            if axis == 0:
                single_view[n] = grid[idx, :, :]
            elif axis == 1:
                single_view[n] = grid[:, idx, :]
            else:
                single_view[n] = grid[:, :, idx]

        return single_view # torch.Size([3, 100, 100]) contsrcuted from 3 slices in a random selected axis
